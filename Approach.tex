...
%
\mdf{To enhance the structural details in the inpainted region, our STSTNet consists of two parts: an edge inpainting network ENet that recovers structural details of the input frames and (b) a spatio-temporal inpainting network STINet that generates plausible contents that looks spatially and temporally consistent. While this forward pass takes multiple frames as input and only completes a single frame, we design a flow inpainting network FNet which predicts the motion flows in the missing regions to enforce the temporal consistency in the inpainted edge maps and videos in a joint learning manner. }
...
To generate plausible video content, a spatio-temporal inpainting network \mdf{(STINet)} is designed to obtain the final target frame $Y_t$.
...
\cxj{Need one sentence to introduce the main concept of SEM here. An attention mechanism? }
...
\mdf{First reconstruction loss between the predicted video frame and the ground truth video frame $\widetilde{Y}_t$.}
Inspired by \cite{nazeri2019edgeconnect}, $\mathcal{L}^{I}_{rec}$ \mdf{measures the difference between .. }. It consists of three terms:
\begin{equation}
