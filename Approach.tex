...
%
To enhance the structural details and temporal coherence in the filled region, our SOVI consists of two flow paths. The first is an edge-guided inpainting path, which consists of an edge inpainting network ENet that recovers missing edges of the input frames, a coarse-to-fine texture inpainting network that replenishes the appearance details.
The second flow path leverages flow information to enhance the temporal coherence of the completed frames.
It consists of a flow inpainting network FNet that predicts the motion flows in the missing regions, a flow-edge consistency loss that jointly constrain flow and edge, and an ensemble module that aggregate previously synthesized frames.%looks spatially and temporally consistent. 
...
To fill the missing regions with fine details, their corresponding structural edges are predicted, which are beneficial to the following frame inpainting process.
% reasonable clues
...
%\begin{figure}[t]
%	\centering
%	\includegraphics[width=1.0\columnwidth]{flow_3} % Reduce the figure size so that it is slightly narrower than the column. Don't use precise values for figure width.This setup will avoid overfull boxes. 
%	\caption{The pipeline of flow inpainting network.}
%	
%	%	. . FNet uses the Resnet101 as backbone, followed by a decoder. STI . In FNet, $T$ is the number of input frames, and channel $2$ denotes the motion along $x$ and $y$ axis. }
%	\label{fig:flow_3}
%\end{figure}
...
Given completed edge maps $\boldsymbol{E}$ for the 
...
%
The proposed TINet consists of a coarse network and a refinement network, as Fig.~\ref{fig:stiNet} shows.
%
...
Second, the refinement network is implemented with 2D convolutional layers to enhance structural details and refine the initial completed frame to produce a visually realistic frame with fine textures. An example of coarse and refine results is shown in Fig.~\ref{fig:coarse-fine}.



To fully exploit the structural information in $\boldsymbol{E}$, a structure attention module (SAM) is designed in the fine texture network.
The core insight of the SAM is to capture the spatial correlation between structural edges and video contents.
%, which is easier to embed into TINet than original edges.
...
After introducing structure information, the inpainted video content by TINet becomes more structurally and semantically realistic.

%\cxj{Can we show a figure to illustrate the initial edge map, the inpainted results of the coarse network, then the refinement network using one example?}


To train the TINet, the reconstruction loss is:
%
\begin{equation}
	\label{eq:1}
...
\end{equation}
where $G^I$ is the TINet and $D^I$ has the similar architecture as that of $D^E$.% $\mathcal{L}^I_{adv}$ enforces the generated frame to be more realistic.
 \begin{figure}[t]
	\centering
...
%\cxj{Move this flow-related loss into the next 
%	section.}
...
\subsection{Temporal Consistency Enhancement}
...
%
Similar to ENet, initial flow maps \(\boldsymbol{O}^i\) for pairs between current frame and neighboring frames are first generated using a flow extraction network, such as FlowNet2.0~\cite{Flownet_2017_CVPR}.
...
As shown in Fig.~\ref{fig:stiNet}, FNet uses Resnet101 as backbone, followed by a decoder.
...
To enhance temporal consistency in the final results, we design a flow-edge consistency loss for edge inpainting and temporal ensemble for inpainted frame with the completed optical flow.
...
Furthermore, to smooth temporal flickers, the motion tendency information in $\boldsymbol{O}$ is utilized to aid the final result via a flow warping constraint and a temporal ensemble module.
