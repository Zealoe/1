...
%
\mdf{As shown in Fig.~\ref{fig:stiNet},} ENet consists of a generator $G^E$ and a discriminator $D^E$.
...
Feature matching loss \mdf{was first proposed in \cite{wang2018high}} and has been widely used in recent GANs.
...
To fully exploit the structural information in $\boldsymbol{E}$, a structure attention module (SAM) is designed in the \mdf{fine texture network}.
The core insight of the SAM is to capture the spatial correlation between structural edges and video contents.
%, which is easier to embed into STINet than original edges.
...
\end{equation}
where $G^I$ is the STINet and $D^I$ has the similar architecture as that of $D^E$.% $\mathcal{L}^I_{adv}$ enforces the generated frame to be more realistic.
