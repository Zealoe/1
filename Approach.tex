...
%\end{equation}
%
%where the first term is the pixel $l_1$ loss to evaluate generation quality. Different from \cite{nazeri2019edgeconnect}, we penalize both coarse and fine predictions, which is defined by:
\begin{equation}
	\begin{aligned}
...
\end{equation}
%
%The second term is the perceptual loss \cite{gatys2015neural}, which compares the semantic contents between the generated frame and ground truth. The third term is the style loss, designed to avoid "checkerboard" artifacts \cite{Sajjadi_2017_ICCV} caused by transpose convolution.
